...
class LcClient:
    ...
    def fetch_submissions(self, pages: list[int]):
        ...
        for page in pages:
            ...
            question_id = data["questions"][self.ques_num-1]["question_id"]
            ...
            for user, submissions in zip(data["total_rank"], data["submissions"]):
                ...
                subm_path = join(self.out_path, rel_code_path)
                ...
                writer.writerow([rel_code_path, subm_time, cont_subm['date']])
                info_file.flush()
        
        info_file.close()

def main():
    client = LcClient("weekly-contest-391", 4)
